...
- 除按新原型接入五个子级操作页外，不改变当前主页面用户操作流程。
- 不改变当前前端展示样式。
- 不改变当前前端业务逻辑。
- 不引入权限。
- 不引入审批。
...
新增子级操作页以纳入版本管理的根目录文件 `飞书敏捷项目管理 · 编辑台账(单文件) (1).html` 为视觉和交互契约。之前原型中的主页面已经实现，本次实施只在现有主页面基础上 1:1 增加 `子级操作页 · 创建与配置` 区域，不重新设计已经实现的主页面。
...
原型中出现的成员选择、经办、共担和群配置只代表当前操作需要的成员关系和通知关系，不引入权限、审批、负责人、责任人或团队模型。原型外层文案中出现的版本历史不纳入实施范围，文档仍不做版本系统。完成这五个子级操作页 1:1 复刻以后，不再允许 UI 重设计，也不再允许前端操作流程重设计。
...
- Issue 事件中的 `issue_id` 和 `actor_member_id` 必须存在。
...
- 飞书通知写入前必须在同一事务中校验 `target_group_id`，以及 payload 中引用的 standup、issue、document 或 comment。
...
- bot 配置按项目全量替换群和规则，不做增量 upsert。
- `PUT /api/feishu/bot-config` 请求必须包含 `project_id`、`groups` 和 `rules`。
- 请求中的既有群和既有规则可以携带已有 id；新建群和新建规则不得携带 id。
- 后端以请求中的 `groups` 和 `rules` 作为该项目的完整目标状态；数据库中未出现在请求里的该项目既有群和规则必须删除。
- 全量替换必须在单个事务中完成；任一群、规则或引用关系校验失败时整次替换拒绝写入。
...
- API 测试证明创建项目、创建工单、创建文档、创建目录、创建评论请求不接受客户端主键 id，并证明 bot 配置全量替换中新建群和新建规则不接受客户端主键 id。
...
- 不做负责人或责任人模型。
- 不做团队模型。
- 除按照 `飞书敏捷项目管理 · 编辑台账(单文件) (1).html` 1:1 新增五个子级操作页外，不做 UI 重设计。
- 五个子级操作页 1:1 复刻完成后，不做前端操作流程重设计。
- 不做猜测性的协作动态流。
