...
contains('pnpm-workspace.yaml', "- 'apps/*'")
contains('package.json', '"db:migrate:remote": "pnpm --filter @agilix/frontend exec wrangler d1 migrations apply agilix-v2-db --remote --config ../../apps/agilix-api/wrangler.toml"')
contains('DEPLOY.md', 'apps/agilix/wrangler.toml')
contains('DEPLOY.md', 'apps/agilix-api/wrangler.toml')
contains('package.json', /"packageManager": "pnpm@9\./)
contains('package.json', '"lint": "eslint \\"{packages,apps}/**/*.{ts,tsx}\\""')
contains('package.json', '"test": "pnpm check:consistency && pnpm -r --if-present test"')
...
contains('apps/agilix/package.json', '"test": "vitest run"')
contains('apps/agilix-api/package.json', '"test": "vitest run"')
