...
export function createTestDrizzleDb() {
  ...
  db.run(sql`create table issue_events (id text primary key, issue_key text not null references issues(key) on delete cascade, type text not null, actor_id text references members(id), message text not null, created_at text not null)`)
  db.run(sql`create table documents (id text primary key, scope text not null, project_id text references projects(id) on delete cascade, title text not null, directory text not null, body text not null, updated_at_label text not null)`)
